...
- Slice 10: no product-contract deviations. The old admin surfaces and Factory-map projection were removed only after the surviving console and shared trees showed no dependency on `map-view`; legacy path redirects remain in `app.routes.ts`. Backend (185), console (46), and shared (86) tests pass. Both required production builds were attempted unchanged under Node 25.6.1 and aborted at `Building...` with exit 134 before Angular emitted a compilation diagnostic; `angular.json` and dependencies were left untouched for the reviewer's pinned/networked toolchain.
