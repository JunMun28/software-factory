...
const STAGE_COLS: { key: string; glyph: string }[] = [
  ...
  { key: 'build', glyph: 'ring' }, { key: 'review', glyph: 'ring' }, { key: 'done', glyph: 'check' },
];
...
export class BCard {
  ...
  open() { this.router.navigate([], { queryParams: { open: this.r().id }, queryParamsHandling: 'merge' }); }
}
...
@Component({
  ...
  template: `
      ...
      @if (d(); as r) {
        <div class="sp-head">
          <div class="row" style="gap:8px">
            <sf-glyph [type]="r.needs_human ? 'flag' : 'ring'" [size]="15" [color]="r.needs_human ? 'var(--red)' : 'var(--a500)'" [fill]="0.4" />
            <span class="chip">{{ typeLabel(r.type) }}</span>
            @if (r.needs_human) { <sf-sig tone="red" glyph="flag">Needs human</sf-sig> }
            @else if (r.gate === 'approve_spec' && !approved()) { <sf-sig tone="amber">Approve spec</sf-sig> }
            @else if (r.gate === 'approve_merge' && !approved()) { <sf-sig tone="amber">Approve merge</sf-sig> }
            @else if (approved() || r.status === 'approved') { <span class="pill purple"><sf-glyph type="ring" [size]="13" color="var(--a600)" [fill]="0.55" />Building</span> }
            <span class="row" style="margin-left:auto;gap:4px">
              <button class="btn ghost sm" title="Open full issue" (click)="openFull(r.id)"><sf-icon name="link" [size]="14" /> Open full issue</button>
              <button class="btn ghost sm" (click)="closed.emit()"><kbd class="kbd">Esc</kbd></button>
            </span>
          </div>
          <h2 style="font-size:21px;margin:8px 0 2px">{{ r.title }}</h2>
          <div style="font-size:12.5px;color:var(--muted)">{{ r.app_name }} · <span class="mono" style="font-size:12px">{{ r.ref }}</span>@if (r.assignee_initials) { · owner {{ r.assignee_initials }} }</div>
        </div>
        <div class="sp-body scroll">
          @if (r.needs_human) {
            <div class="openq" style="margin-bottom:14px;border-color:#E7AEA7;background:var(--red-bg)">
          ...
          <div class="block" style="border-left:3px dashed var(--border-strong)">
            <div style="font-size:12.5px;color:var(--fg2)">
              ...
              <span [style.color]="r.spec_pr_open ? 'var(--green)' : 'var(--muted)'">SPEC.md PR {{ r.spec_pr_open ? '✓' : '…' }}</span> →
              <span [style.color]="r.stage2_fired ? 'var(--green)' : 'var(--muted)'">Stage 2 {{ r.stage2_fired ? '✓' : '…' }}</span>
        ...
        <div class="sp-foot">
          @if (approved()) {
            <span class="row" style="gap:8px;font-size:13.5px;color:var(--green-tx)"><sf-glyph type="check" [size]="16" color="var(--green)" /> Approved — build started. Watch it in the feed.</span>
            ...
            <button class="btn sm" style="margin-left:auto;border-style:dashed;color:var(--muted)" (click)="cancel(r)">Cancel</button>
          } @else if (r.gate) {
            <button class="btn primary" (click)="confirming.set(true)">Approve <kbd class="kbd">A</kbd></button>
            @if (r.gate === 'approve_spec') { <button class="btn" (click)="openFull(r.id)">Send back <kbd class="kbd">S</kbd></button> }
            <button class="btn sm" style="margin-left:auto;border-style:dashed;color:var(--muted)" (click)="cancel(r)">Cancel <kbd class="kbd">C</kbd></button>
          } @else {
    ...
    @if (confirming()) {
      <div class="palette-scrim" style="align-items:center;padding-top:0;z-index:50" (click)="confirming.set(false)">
        <div class="palette" style="width:460px;padding:22px 24px;align-self:center" (click)="$event.stopPropagation()">
          <h3 style="font-size:19px;margin-bottom:8px">{{ d()?.gate === 'approve_merge' ? 'Approve this merge?' : 'Approve this spec?' }}</h3>
          <p style="font-size:14px;color:var(--muted);margin:0 0 4px">Approving <b style="color:var(--fg1)">{{ d()?.title }}</b> is irreversible. It will:</p>
          ...
          <div class="row" style="gap:9px;justify-content:flex-end">
            ...
            <button class="btn primary" (click)="approve()">{{ d()?.gate === 'approve_merge' ? 'Approve & deploy' : 'Approve & start build' }}</button>
          </div>
        </div>
      </div>
    }
...
export class DetailPanel {
  ...
  approve() {
    ...
    this.api.approve(r.id, this.session.user().name).subscribe({
      next: (d) => { this.d.set(d as RequestDetail); this.poll.nudge(); },
      error: () => this.approved.set(false),
    });
  }
  retry(r: RequestDetail) {
    this.api.retry(r.id, this.session.user().name).subscribe(() => { this.poll.nudge(); this.closed.emit(); });
  }
  cancel(r: RequestDetail) {
    this.api.cancel(r.id, this.session.user().name).subscribe(() => { this.poll.nudge(); this.closed.emit(); });
  }
  openFull(id: number) { this.router.navigateByUrl(`/admin/issue/${id}`); }
}
    ...
    <admin-shell active="board" title="Board">
      ...
      <span headerExtra class="row" style="gap:11px">
        ...
        <div style="position:relative">
          <button class="kpill" style="padding:5px 10px;font-size:12.5px" (click)="menu.set(!menu())">
            <span style="color:var(--faint)">Group by:</span> <span style="color:var(--fg1);font-weight:600">{{ groupLabel() }}</span>
            <sf-icon name="chevDown" [size]="13" color="var(--faint)" />
          </button>
          @if (menu()) {
            <div style="position:fixed;inset:0;z-index:19" (click)="menu.set(false)"></div>
            <div style="position:absolute;top:calc(100% + 5px);left:0;z-index:20;width:184px;background:var(--surface);border:1px solid var(--border);border-radius:9px;box-shadow:var(--shadow-pop);overflow:hidden;padding:5px">
              <div style="font-size:10px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--faint);padding:5px 9px 4px">Swimlanes by</div>
              @for (g of groupOpts; track g[0]) {
                <button (click)="groupBy.set($any(g[0])); menu.set(false)"
                  ...
                  <sf-icon [name]="g[2]" [size]="15" [color]="groupBy() === g[0] ? 'var(--a600)' : 'var(--muted)'" />
                  <span style="flex:1">{{ g[1] }}</span>
                  @if (groupBy() === g[0]) { <sf-icon name="check" [size]="15" color="var(--a600)" /> }
                </button>
              }
            </div>
          }
          ...
          @for (col of cols; track col.key) {
            <div class="col" [class.inert]="inert(col.key)">
              ...
              <div class="col__body scroll">
                @for (r of byStage(col.key); track r.id) { <sf-bcard [r]="r" [sel]="r.gate === 'approve_spec' && r.id === firstGateId()" /> }
              </div>
            </div>
          }
        </div>
          ...
          @for (lane of lanes(); track lane.key) {
            <div class="swim">
              <button class="swim__head" (click)="toggleLane(lane.key)">
                <sf-icon [name]="laneOpen(lane.key) ? 'chevDown' : 'chevRight'" [size]="14" color="var(--muted)" />
                @if (groupBy() === 'app') { <span style="color:var(--faint);font-weight:600">#</span> }
                @if (groupBy() === 'owner') {
                  @if (lane.av) { <sf-avatar [sm]="true" [color]="lane.color ?? '#7A6E9A'">{{ lane.av }}</sf-avatar> }
                  @else { <span class="swim__unassigned"></span> }
                }
                ...
                <div class="swim__row">
                  @for (col of cols; track col.key) {
                    <div class="swim__cell" [class.inert]="inert(col.key)">
                      @for (r of laneCell(lane, col.key); track r.id) { <sf-bcard [r]="r" /> }
                    </div>
                  }
                </div>
              }
...
export class Board {
  ...
  groupLabel() { return ({ none: 'None', app: 'App', owner: 'Assignee', type: 'Type' } as Record<string, string>)[this.groupBy()]; }
  ...
  lanes = computed(() => {
    ...
    const labelOf = (k: string) =>
      g === 'type' ? ({ bug: 'Bug fix', enh: 'Enhancement', new: 'New app', other: 'Other' } as Record<string, string>)[k] ?? k : k;
    ...
    return [...map.entries()].map(([key, items]) => ({
      key, label: labelOf(key), items,
      av: g === 'owner' ? items.find((r) => r.assignee_initials)?.assignee_initials ?? null : null,
      color: g === 'owner' ? items.find((r) => r.assignee_color)?.assignee_color ?? null : null,
    }));
  });
  laneCell(lane: { items: FactoryRequest[] }, stage: string) { return lane.items.filter((r) => r.stage === stage); }
  laneOpen(key: string) { return !this.closedLanes().has(key); }
  ...
  closePanel() { this.router.navigate([], { queryParams: { open: null }, queryParamsHandling: 'merge' }); }
}
