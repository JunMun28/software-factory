...
interface Group { key: string; label: string; count: number; items: FactoryRequest[]; collapsed?: boolean }
...
@Component({
        ...
        <div style="max-width:880px;margin:0 auto;padding:0 22px">
          ...
          <div class="pipe-legend">
            @for (s of stageShort; track s; let i = $index) {
              ...
              @if (i === 1 || i === 4) { <span class="pipe-legend__gate" [title]="i === 1 ? 'Spec gate' : 'Merge gate'"></span> }
            }
          </div>
              ...
              @for (r of g.items; track r.id) {
                <div class="pipe-row focusable" tabindex="0" role="button" [class.pipe-row--red]="r.needs_human"
                  ...
                  <div class="pipe-row__main">
                    ...
                    @if (r.needs_human) { <sf-sig tone="red" glyph="flag" (click)="toQueue($event)">Needs human</sf-sig> }
                    @else if (r.gate === 'approve_spec') { <sf-sig tone="amber" kbd="A" (click)="toQueue($event)">Approve spec</sf-sig> }
                    @else if (r.gate === 'approve_merge') { <sf-sig tone="amber" kbd="A" (click)="toQueue($event)">Approve merge</sf-sig> }
                    @else if (r.status === 'sent_back') { <span class="chip">with submitter</span> }
                    @else if (r.status === 'done') { <span class="chip" style="color:var(--green-tx)">deployed</span> }
                    @else if (r.status === 'cancelled') { <span class="chip">cancelled</span> }
                    @else if (r.status === 'approved') { <span style="font-size:11.5px;color:var(--a700)">{{ stageShort[stageIdx(r)] }} · agents working</span> }
                    @else { <span class="chip">triage</span> }
                  </div>
                  <div class="pipe-strip">
                    @for (seg of strip(r).segs; track $index; let i = $index) {
                      <span class="pipe-seg" [class]="'pipe-seg pipe-seg--' + seg"></span>
                      @if (i === 1) { <span class="pipe-gate" [class]="'pipe-gate pipe-gate--' + strip(r).specGate"></span> }
                      @if (i === 4) { <span class="pipe-gate" [class]="'pipe-gate pipe-gate--' + strip(r).mergeGate"></span> }
                    }
                  </div>
                  <div class="pipe-row__meta">
                    <span class="row" style="gap:5px"><sf-icon name="clock" [size]="13" /> {{ clockLine(r) }}</span>
                    @if (r.last_event) { <span class="pipe-row__event">"{{ r.last_event }}"</span> }
                  </div>
                </div>
              }
            }
  ...
  `,
  styles: `
    .pipe-legend { display:flex; align-items:center; gap:4px; padding:0 16px 7px; }
    .pipe-legend__seg { flex:1; text-align:center; font-size:10px; font-weight:600; letter-spacing:.08em; text-transform:uppercase; color:var(--faint); }
    .pipe-legend__gate { width:9px; height:9px; flex:0 0 9px; transform:rotate(45deg); border:1.5px solid var(--border-strong); border-radius:1px; }
    .pipe-grouphead { display:flex; align-items:center; gap:6px; font-size:10.5px; font-weight:600; letter-spacing:.09em; text-transform:uppercase; color:var(--faint); margin:16px 2px 7px; }
    ...
    .pipe-seg { flex:1; height:5px; border-radius:3px; background:var(--surface-3); }
    .pipe-seg--done { background:var(--a400); }
    .pipe-seg--active { background:repeating-linear-gradient(90deg, var(--a400) 0 6px, var(--a100) 6px 12px); background-size:200% 100%; animation:pipeflow 1.6s linear infinite; }
    .pipe-seg--off { background:var(--surface-2); }
    @keyframes pipeflow { to { background-position:-24px 0; } }
    @media (prefers-reduced-motion: reduce) { .pipe-seg--active { animation:none; } }
    .pipe-gate { width:9px; height:9px; flex:0 0 9px; transform:rotate(45deg); border-radius:1px; background:var(--border-strong); }
    .pipe-gate--passed { background:var(--a600); }
    .pipe-gate--waiting { background:var(--amber); box-shadow:0 0 0 3px var(--amber-bg); }
    .pipe-gate--escalated { background:var(--red); box-shadow:0 0 0 3px var(--red-bg); }
    .pipe-gate--submitter { background:var(--surface); border:1.5px solid var(--amber-line); }
    .pipe-row__meta { display:flex; align-items:center; gap:14px; margin-top:7px; font-size:11.5px; color:var(--muted); }
    .pipe-row__event { color:var(--faint); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:0; }
  `,
})
export class Pipeline {
  ...
  groups = computed<Group[]>(() => {
    const rs = this.all();
    const needsMe = rs.filter((r) => r.gate || r.needs_human).sort((a, b) => Number(b.needs_human) - Number(a.needs_human));
    const inFlight = rs.filter((r) => !r.gate && !r.needs_human && r.status === 'approved' && ['architecture', 'build', 'review'].includes(r.stage));
    const triage = rs.filter((r) => !r.needs_human && r.status === 'submitted' && r.stage === 'intake');
    ...
    const defs: Group[] = [
      ...
      { key: 'flight', label: 'In flight — agents working', count: inFlight.length, items: inFlight },
      ...
      { key: 'closed', label: 'Done & closed', count: closed.length, items: closed, collapsed: true },
    ];
    return defs.filter((g) => g.count > 0);
  });

  stageIdx(r: FactoryRequest) { return Math.max(0, STAGES.indexOf(r.stage)); }
  ...
  strip(r: FactoryRequest): Strip {
    ...
    const specGate: Strip['specGate'] =
      r.needs_human && idx <= 1 ? 'escalated'
      : r.gate === 'approve_spec' ? 'waiting'
      : r.status === 'sent_back' ? 'submitter'
      : idx >= 2 || closed ? 'passed' : 'pending';
  ...
  isOpen(key: string) { return this.openGroups().has(key); }
  toggle(key: string) {
