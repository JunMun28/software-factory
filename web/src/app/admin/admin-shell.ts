import { Component, Directive, ElementRef, HostListener, afterNextRender, computed, inject, input, signal } from '@angular/core';
  ...
  template: `
    <div class="adm">
      <aside class="adm-side">
        <div class="adm-brand"><sf-mark [size]="18" /><span class="adm-brand__name">Factory</span></div>

        <button class="btn primary" style="width:100%;justify-content:flex-start;margin-top:12px;margin-bottom:4px" (click)="newIssue.set(true)">
          <sf-icon name="plus" [size]="16" /> New issue <kbd class="kbd" style="margin-left:auto">C</kbd>
        </button>
        ...
        <button class="navrow" [class.on]="active() === 'list'" (click)="go('/admin/list')">
          <span class="navrow__ic"><sf-icon name="list" [size]="17" /></span><span class="navrow__label">List</span>
        ...
        <button class="navrow" [class.on]="active() === 'needsme'" (click)="go('/admin/inbox')">
          <span class="navrow__ic"><sf-icon name="inbox" [size]="17" /></span><span class="navrow__label">Needs me</span>
          @if (redCount()) { <span class="navrow__count red">{{ redCount() }}</span> }
          @else if (gateCount()) { <span class="navrow__count amber">{{ gateCount() }}</span> }
        ...
        <button class="navrow" [class.on]="active() === 'queue'" (click)="go('/admin/queue')">
          <span class="navrow__ic"><sf-glyph type="ring" [size]="15" [fill]="0.4" /></span><span class="navrow__label">Approval queue</span>
        ...
        @for (a of apps(); track a.id) {
          <button class="navrow" [class.on]="active() === 'feed:' + a.key" (click)="go('/admin/apps/' + a.key)">
            <span class="navrow__ic">@if (a.muted) { <sf-icon name="mute" [size]="14" /> } @else { <sf-icon name="hash" [size]="15" /> }</span>
            <span class="navrow__label" [style.opacity]="a.muted ? 0.55 : 1">{{ a.name }}</span>
            @if (a.unread) { <span class="navrow__dot"></span> }
          </button>
        }

        <div style="margin-top:auto;padding-top:10px;border-top:1px solid var(--border)">
          <button class="navrow" [class.on]="active() === 'registry'" (click)="go('/admin/registry')">
            <span class="navrow__ic"><sf-icon name="app" [size]="17" /></span><span class="navrow__label">App registry</span>
            <span class="navrow__tip">Registry <kbd class="kbd">G</kbd><kbd class="kbd">R</kbd></span>
          </button>
          <button class="navrow" [class.on]="active() === 'settings'" (click)="go('/admin/settings')">
      ...
      <div class="adm-main">
        ...
        <div class="adm-canvas">
              ...
              <div class="palette" (click)="$event.stopPropagation()">
                ...
                <div style="padding:6px">
                  ...
                  @for (a of filteredActions(); track a.lbl; let i = $index) {
                    <div class="palette__row" [class.on]="palSel() === i" (mouseenter)="palSel.set(i)" (click)="runPalette(a)">
                      <sf-icon [name]="a.icon" [size]="15" color="var(--muted)" />
                      <span class="palette__lbl">{{ a.lbl }}</span>
                      @if (a.hint) { <kbd class="kbd">{{ a.hint }}</kbd> }
                    </div>
                  }
                  <div class="palette__group">Jump to</div>
                  @for (a of filteredJumps(); track a.lbl; let i = $index) {
                    <div class="palette__row" [class.on]="palSel() === i + filteredActions().length"
                      (mouseenter)="palSel.set(i + filteredActions().length)" (click)="runPalette(a)">
                      <sf-icon [name]="a.icon" [size]="15" color="var(--muted)" />
                      <span class="palette__lbl">{{ a.lbl }}</span>
                    </div>
                  }
          ...
          @if (cheats()) {
            <div class="palette-scrim" style="align-items:center;padding-top:0" (click)="cheats.set(false)">
              <div class="palette" style="width:540px;padding:20px 24px;align-self:center" (click)="$event.stopPropagation()">
                <div class="row" style="justify-content:space-between;margin-bottom:16px">
                  <h3 style="font-size:19px">Keyboard shortcuts</h3><span style="font-size:12px;color:var(--faint)">esc to close</span>
                </div>
                  ...
                  <div style="flex:1;min-width:0">
                    ...
                    <div style="display:flex;flex-direction:column;gap:9px">
                      @for (row of cheatNav; track row[0]) {
                        <div class="row" style="gap:8px"><span style="flex:1;font-size:13px;color:var(--fg2)">{{ row[0] }}</span>
                          <span class="row" style="gap:4px">@for (k of row[1].split(' '); track $index) { <kbd class="kbd">{{ k }}</kbd> }</span></div>
                      }
                    </div>
                  </div>
                  <div style="flex:1;min-width:0">
                ...
                <p style="font-size:12px;color:var(--faint);margin:0;line-height:1.45">Every action is reachable by mouse, the palette, or a key — the same three ways, everywhere.</p>
              </div>
            </div>
          }
          ...
          <!-- new-issue modal -->
          @if (newIssue()) {
            <div class="palette-scrim" style="align-items:flex-start;padding-top:76px" (click)="newIssue.set(false)">
              <div class="palette" style="width:580px;max-width:94%" (click)="$event.stopPropagation()">
                <div class="row" style="gap:9px;padding:14px 18px 12px;border-bottom:1px solid var(--border)">
                  <span style="font-size:12.5px;color:var(--muted)">New issue in</span>
                  <div style="position:relative">
                    <button class="btn ghost sm" style="gap:5px" (click)="niAppsOpen.set(!niAppsOpen())">
                      <span style="color:var(--faint)">#</span> {{ niAppName() }} <sf-icon name="chevDown" [size]="13" />
                    </button>
                    @if (niAppsOpen()) {
                      <div style="position:absolute;top:calc(100% + 4px);left:0;z-index:20;width:230px;background:var(--surface);border:1px solid var(--border);border-radius:8px;box-shadow:var(--shadow-pop);padding:5px">
                        @for (a of apps(); track a.id) {
                          <button style="display:flex;width:100%;text-align:left;padding:7px 10px;border:none;border-radius:6px;background:none;cursor:pointer;font-family:var(--body);font-size:13.5px;gap:8px"
                            (click)="niApp.set(a.id); niAppsOpen.set(false)"><span style="color:var(--faint)">#</span>{{ a.name }}</button>
                        }
                      </div>
                    }
                  </div>
                  <button class="btn ghost sm" style="margin-left:auto" (click)="newIssue.set(false)"><kbd class="kbd">Esc</kbd></button>
                </div>
                <div style="padding:16px 18px">
                  <div class="seg" style="margin-bottom:14px">
                    @for (t of niTypes; track t[0]) {
                      <button [class.on]="niType() === t[0]" (click)="niType.set(t[0])" style="display:inline-flex;align-items:center;gap:6px">
                  ...
                  <div class="row" style="gap:7px;flex-wrap:wrap;margin-top:6px">
                    <span class="chip" style="gap:6px"><sf-avatar [sm]="true" color="#6E5A8A">KP</sf-avatar> Kim P.</span>
                    <span class="chip" style="gap:6px"><sf-icon name="chevUp" [size]="13" color="var(--amber)" /> Normal</span>
                    <span class="chip lbl--add" style="gap:5px"><sf-icon name="plus" [size]="12" /> Label</span>
                    <span class="chip" style="gap:6px"><sf-icon name="image" [size]="13" /> Attach</span>
                  </div>
                </div>
                <div class="row" style="gap:9px;padding:12px 18px;border-top:1px solid var(--border);justify-content:flex-end">
                  <button class="btn" (click)="newIssue.set(false)">Cancel</button>
                  <button class="btn primary" [disabled]="!niTitle().trim()" (click)="createIssue()">Create issue <kbd class="kbd">↵</kbd></button>
                </div>
              </div>
            </div>
          }
...
export class AdminShell {
  ...
  createIssue() {
    ...
    this.api.createRequest({
      type: this.niType(), title: this.niTitle().trim(), description: this.niDesc().trim(),
      ...
      this.api.submit(r.id).subscribe(() => {
        ...
        this.niTitle.set(''); this.niDesc.set('');
        this.poll.nudge();
  ...
  paletteActions = [
    ...
    { icon: 'refresh', lbl: 'Run factory tick (simulate CI)', hint: '', act: () => this.api.tick().subscribe(() => this.poll.nudge()) },
  ];
  ...
  filteredJumps = computed(() => {
    ...
    return this.apps()
      ...
      .map((a) => ({ icon: 'hash', lbl: a.name, hint: '', act: () => this.go('/admin/apps/' + a.key) }));
  });
  ...
  runPalette(a: { act: () => void }) { a.act(); this.paletteOpen.set(false); this.query.set(''); this.palSel.set(0); }
