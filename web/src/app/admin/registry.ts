...
@Component({
  ...
  template: `
    <admin-shell active="registry" title="App registry">
      <span headerRight>
        <button class="btn primary sm" (click)="startNew()">
          <sf-icon name="plus" [size]="14" /> New
        </button>
      </span>
      <div class="regtable scroll">
        <div class="reghead">
          <span>App / channel</span><span>Owner</span><span>Repo</span><span>Source</span
          ><span></span>
        </div>
        <div [style.opacity]="editing() !== null ? 0.55 : 1">
          @for (a of apps(); track a.id) {
            <div
              class="regrow focusable"
              tabindex="0"
              role="button"
              [class.focus]="editing()?.id === a.id"
              (click)="edit(a)"
              (keydown.enter)="edit(a)"
            >
              <span class="row" style="gap:7px"
                ...
                @if (a.unread) {
                  <span
                    style="width:6px;height:6px;border-radius:50%;background:var(--a500)"
                  ></span>
                }
              </span>
              ...
              <span style="color:var(--faint);text-align:right"
                ><sf-icon name="chevRight" [size]="15"
              /></span>
            </div>
          }
        </div>
        ...
        <div class="sidepanel fade-in" style="width:360px">
          <div class="sp-head row" style="gap:7px">
            ...
            @if (isNew()) {
              <input
                ...
                [(ngModel)]="form.name"
                style="min-height:34px;padding:5px 10px;font-weight:700"
              />
            } @else {
              <span style="font:700 18px/1 var(--display)">{{ e.name }}</span>
            }
            <button class="btn ghost sm" style="margin-left:auto" (click)="close()">
          ...
          <div class="sp-body scroll" style="display:flex;flex-direction:column;gap:14px">
            <div>
              ...
              ><input class="input" [(ngModel)]="form.owner" placeholder="team-name" />
            </div>
            <div>
              ...
              <div class="input focus">
                ...
                />
                <span class="row" style="margin-left:auto;gap:5px;color:var(--green);font-size:12px"
                  ><sf-glyph type="check" [size]="14" color="var(--green)" /> verified</span
                >
              </div>
            </div>
            @if (!isNew() && e.open_requests > 0) {
              <div
                style="border:1px dashed var(--border-strong);border-radius:8px;padding:11px 13px;font-size:12.5px;color:var(--muted)"
              >
                Changing this feeds <b style="color:var(--fg1)">1 channel</b>, linked to
                <b style="color:var(--fg1)"
                  >{{ e.open_requests }} request{{ e.open_requests > 1 ? 's' : '' }}</b
                >.
              </div>
            }
          </div>
