...
@Component({
  ...
  template: `
    <sub-shell active="new" [step]="2" [reqId]="id">
      <div class="sub-col fade-in">
        ...
        <p style="color:var(--muted);margin:6px 0 22px;font-size:16px">Here's what we'll send to the reviewer. Go back to any step to change it.</p>
        @if (req(); as r) {
          <div class="card" style="overflow:hidden">
            ...
            <div class="summ-row">
              ...
              <div class="summ-v">{{ r.app_name }}</div><span></span>
            </div>
            <div class="summ-row">
            ...
            @for (t of answered(r); track t.order) {
              <div class="summ-row">
                ...
                <button class="btn ghost sm" style="color:var(--accent-link)" (click)="go('/submit/' + id + '/interview')">Edit</button>
              </div>
            }
            @if (extra) {
              <div class="summ-row"><span class="summ-k">Extra details</span><div class="summ-v">{{ extra }}</div><span></span></div>
            }
        ...
        <div class="row" style="justify-content:space-between;margin-top:22px">
          <button class="btn ghost" (click)="go('/submit/' + id + '/interview')"><sf-icon name="back" [size]="16" /> Back</button>
          <button class="btn primary lg" [disabled]="submitting()" (click)="submit()">
            {{ submitting() ? 'Submitting…' : 'Submit request' }} <sf-icon name="arrowRight" [size]="16" />
          </button>
        </div>
      </div>
    </sub-shell>
  `,
  styles: `
    .summ-row { display:flex; gap:16px; padding:14px 18px; border-bottom:1px solid var(--hairline); align-items:flex-start; }
    .summ-k { width:160px; flex:0 0 160px; font-size:13px; color:var(--muted); padding-top:2px; }
    .summ-v { flex:1; min-width:0; font-size:14.5px; color:var(--fg1); line-height:1.5; }
  `,
})
export class Review {
