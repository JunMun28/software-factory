  ...
  template: `
    <sub-shell active="list">
      <div class="sub-col fade-in">
        <div
          class="row"
          style="justify-content:space-between;align-items:flex-end;margin-bottom:18px"
        >
        ...
        <div style="display:flex;flex-direction:column;gap:11px">
          @for (r of needsInput(); track r.id) {
            <div class="attn lift fade-in" style="display:flex;flex-direction:column;gap:10px">
              <div class="row" style="gap:9px">
                <sf-sig tone="amber" glyph="flag">Needs your input</sf-sig
                ><span style="font-size:12px;color:var(--amber-tx)"
              ...
              <div>
                <button class="btn primary sm" (click)="go('/requests/' + r.id)">
                  Respond <sf-icon name="arrowRight" [size]="15" />
                </button>
              </div>
            </div>
          }
          @for (r of rows(); track r.id) {
            ...
            >
              <div class="reqrow__main">
                ...
                <div class="reqrow__meta">
                  <sf-type-chip [t]="r.type" /><span>{{ r.app_name }} · {{ age(r) }}</span>
                </div>
              </div>
            ...
            <div style="text-align:center;padding:30px;color:var(--faint);font-size:14px">
              Nothing here yet — file your first request.
            </div>
          }
          @if (rows().length) {
            <div style="text-align:center;font-size:13px;color:var(--faint);margin-top:8px">
              — That's everything —
            </div>
          }
        </div>
